...
fn hkdf_derive(sig: &WalletSignature, password: &Password) -> Result<AesKey<256>, DriveKeyError> {
    let sig = sig.as_blob();
...
impl DriveKey {
    pub fn derive_from(
        ...
        let rsa_sk = match wallet.danger_expose_signing_key() {
            SigningKey::Rsa4096(sk) => sk,
            _ => Err(DriveKeyError::UnsupportedKey)?,
        };

        let sig_v1 = v1_wallet_sig(drive_id, rsa_sk)?;
        let sig_v2 = v2_wallet_sig(drive_id, wallet, rsa_sk)?;

        Ok(Self {
            v1: hkdf_derive(&sig_v1, &password)?,
            v2: hkdf_derive(&sig_v2, &password)?,
        })
    }
...
fn derive_file_key(
    file_id: &FileId,
    aes_drive_key: &AesKey<256>,
) -> Result<AesKey<256>, FileKeyError> {
    let mut buf = vec![0u8; 32].into_boxed_slice().confidential();
    let hkdf = Hkdf::<Sha256>::new(None, aes_drive_key.danger_reveal_raw_key().as_slice());
    hkdf.expand(file_id.as_ref(), buf.reveal_mut())?;
    Ok(AesKey::try_from_bytes(buf.reveal()).ok_or_else(|| FileKeyError::AesKeyError)?)
}
...
impl FileKey {
    pub fn derive_from(file_id: &FileId, drive_key: &DriveKey) -> Result<Self, FileKeyError> {
        Ok(Self {
            v1: derive_file_key(file_id, &drive_key.v1)?,
            v2: derive_file_key(file_id, &drive_key.v2)?,
        })
    }
...
impl MetadataCryptor<'_> for DefaultMetadataCryptor {
    ...
    fn decrypt(
        ...
        let (ciphertext, tag, candidates) =
            prepare_encrypted_metadata(encrypted_metadata, self.signature_format)?;

        for sf in candidates {
            if let Ok(plaintext) = drive_key.decrypt_metadata(ciphertext, &self.nonce, &tag, sf) {
                return Ok(plaintext);
            }
        }
        Err(MetadataCryptorError::DecryptionFailed)
    }
}
...
impl<'a> MetadataCryptor<'a> for FileMetadataCryptor<'a> {
    ...
    fn decrypt(
        ...
        let file_key = FileKey::derive_from(self.file_id, drive_key)?;

        let (ciphertext, tag, candidates) =
            prepare_encrypted_metadata(encrypted_metadata, self.signature_format)?;

        for sf in candidates {
            if let Ok(plaintext) = file_key.decrypt_metadata(ciphertext, &self.nonce, &tag, sf) {
                return Ok(plaintext);
            }
        }
        Err(MetadataCryptorError::DecryptionFailed)
...
fn prepare_encrypted_metadata(
    encrypted_metadata: &[u8],
    signature_format: Option<SignatureFormat>,
) -> Result<(&[u8], AesGcmTag<U16>, Vec<SignatureFormat>), MetadataCryptorError> {
    ...
    // if we don't know the signature at this point, try all of them
    let candidates = signature_format
        .map(|sf| vec![sf])
        .unwrap_or_else(|| vec![SignatureFormat::V2, SignatureFormat::V1]);

    Ok((ciphertext, tag, candidates))
}
