...
use crate::serde::{DefaultSerdeStrategy, StringifySerdeStrategy};
...
trait Id: Clone + Debug + PartialEq + PartialOrd + Hash + Send + Sync {}
impl<T> Id for T where T: Clone + Debug + PartialEq + PartialOrd + Hash + Send + Sync + 'static {}

pub type TypedId<T, ID, SER = DefaultSerdeStrategy, STR = DefaultStringify<ID>> =
    Typed<T, ID, SER, STR>;

impl<T, ID: Id, SER, STR> Debug for TypedId<T, ID, SER, STR>
where
    Self: Display,
{
...
pub type TypedBigUintB64Id<T> = TypedId<T, BigUint>;

mod tests {
    use crate::id::{Typed256B64Id, TypedUuid};
    use std::str::FromStr;
